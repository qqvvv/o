...
(() => {
  ...
  const method = async () => {
    ...
    const behaviours = {
      ...
      '层二功能1': async () => {
        const live2d = await refer.adDlibs("https://unpkg.com/live2d-widget@3.1.4/lib/L2Dwidget.min.js");
        try {
          L2Dwidget.init({
    ...
    const getImageUrls = async () => {
        ...
        const arrSrs = allImages.map(trimQueryPara(img => img.src));
        return arrSrs;
    };
